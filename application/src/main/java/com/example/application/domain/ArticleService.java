...
class ArticleService implements PublishArticleUseCase, EditArticleUseCase, DeleteArticleUseCase, GetArticleQuery, UnfavoriteArticleUseCase, FavoriteArticleUseCase, FindFavoriteUseCase {
  ...
  public Article publishArticle(PublishArticleCommand articleToPublish) {
    ...
    Article article =
        Article.builder()
            ...
            .build();
    Article created = createArticlePort.createArticle(article);
  ...
  public Article getArticle(String slug, Optional<User> requester) {
    ...
    Article article = this.findArticlePort.findArticle(slug).orElseThrow(ArticleNotFoundException::new);

    Profile profile = getProfileQuery.getProfile(article.getAuthor().getUsername(), requester);
    if(requester.isPresent()) {
      Boolean isFavorited = this.isFavoritedPort.isArticleFavoritedBy(article.getId(), requester.get().getId());
  ...
  @Override
  public List<Article> getRecentArticles(Optional<List<String>> tags, Optional<String> author, Optional<Boolean> favorited, Optional<User> requester) {
  ...
  public Article favoriteArticle(String slug, User requester) {
    ...
    this.favoritePort.addFavorite(found.getId(), requester.getId());
    return this.getArticle(slug, Optional.of(requester));
  }
  ...
  public Article unfavoriteArticle(String slug, User requester) {
    Article found = this.getArticle(slug, Optional.of(requester));
    this.favoritePort.removeFavorite(found.getId(), requester.getId());
    return this.getArticle(slug, Optional.of(requester));
  }
